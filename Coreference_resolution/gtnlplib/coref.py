...
def get_distances(markables, string):
    ...
    for i in range(len(markables)):
        ...
        markable_str = ' '.join(markables[i]['string']).lower()

        if string == markable_str:
            for entity in entities:
                if i in entity:
                    index = entity.index(i)
                    if index == 0:
                        int_list.append(index)
                        break
                    else:
                        int_list.append(i - entity[entity.index(i) - 1])
                        break
...
def get_tp(pred_ant,markables):
    ...
    entities = get_entities(markables)
    tp_list = []
    for i,pred in enumerate(pred_ant):
        found = False
        if pred < i:
            for entity in entities:
                if pred in entity and i in entity:
                    # print pred
                    # print i
                    # print entity
                    tp_list.append(True)
    ...
    for i, pred in enumerate(pred_ant):
        ...
        for entity in entities:
            if i in entity and entity.index(i) >=1:
                if pred == i or pred not in entity:
                    fn_list.append(True)
                else:
                    fn_list.append(False)

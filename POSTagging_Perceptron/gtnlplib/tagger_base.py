...
def classifier_tagger(tokens,feat_func,weights,all_tags):
    ...
    for i,token in enumerate(tokens):
        ...
        scores_dict = {}
        for tag in all_tags:
            token_dict.update(feat_func(tokens, tag, 'IGNORE',i))
            scores_dict[tag] = 0.0

        for key in token_dict.keys():
            scores_dict[key[0]] += token_dict[key] * weights[key]

        tags.append(argmax(scores_dict))
...
def compute_features(tokens,tags,feat_func):
    ...
    """
    feats = dict()
    feats_default = defaultdict(float)

    M = len(tokens)
    for i in range(1,M):
        temp = feat_func(tokens, tags[i], tags[i-1], i)
        for key in temp.keys():
            feats_default[key] += temp[key]

    temp = feat_func(tokens, tags[0], START_TAG, 0)
    for key in temp.keys():
        feats_default[key] += temp[key]

    temp = feat_func(tokens, END_TAG, tags[M-1], M)
    for key in temp.keys():
        feats_default[key] += temp[key]

    feats = dict(feats_default)
    return feats

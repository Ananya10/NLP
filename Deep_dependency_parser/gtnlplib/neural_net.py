...
class VanillaWordEmbeddingLookup(nn.Module):
    ...
    def forward(self, sentence):
        ...
        # STUDENT
        for input in inp:
            result = self.word_embeddings(input)
...
class BiLSTMWordEmbeddingLookup(nn.Module):
    ...
    def __init__(self, word_to_ix, word_embedding_dim, hidden_dim, num_layers, dropout):
        ...
        self.lstm = nn.LSTM(word_embedding_dim, hidden_dim/2, num_layers, bidirectional=True)
        # END STUDENT
    ...
    def forward(self, sentence):
        ...
        # embeddings contain the tensor of shape (len(sentence_idxs), 1, embedding_dim)
        embeddings = torch.stack(embeds)
        lstm_out, self.hidden = self.lstm(embeddings, self.hidden)
        return_list = list(torch.unbind(lstm_out))
...
class LSTMCombinerNetwork(nn.Module):
    ...
    def __init__(self, embedding_dim, num_layers, dropout):
        ...
        # STUDENT
        self.lstm = nn.LSTM(2*embedding_dim, embedding_dim, num_layers, dropout=dropout)
        # END STUDENT
    ...
    def forward(self, head_embed, modifier_embed):
        ...
        """
        # STUDENT
        flat_input = utils.concat_and_flatten([head_embed,modifier_embed]).view(1,1,-1)
        lstm_out, self.hidden = self.lstm(flat_input, self.hidden)
        return lstm_out.view(1,self.embedding_dim)
        # END STUDENT
...
class ActionChooserNetwork(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.layer2 = nn.ReLU()
        self.layer3 = nn.Linear(input_dim, 3)

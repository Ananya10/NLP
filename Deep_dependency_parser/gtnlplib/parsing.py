...
class ParserState:
    ...
    def done_parsing(self):
        ...
        """
        # STUDENT
        if len(self.stack) == 1 and self.input_buffer_peek_n(1)[0].headword == END_OF_INPUT_TOK:
            return True
    ...
    def _reduce(self, action):
        ...
        if action == Actions.REDUCE_R:
            result = self.combiner(item2.embedding,item1.embedding)
            self.stack.append(StackEntry(item2.headword, item2.headword_pos, result))
            edge = DepGraphEdge((item2.headword, item2.headword_pos), (item1.headword, item1.headword_pos))
        elif action == Actions.REDUCE_L:
            result = self.combiner(item1.embedding,item2.embedding)
            self.stack.append(StackEntry(item1.headword, item1.headword_pos, result))
            edge = DepGraphEdge((item1.headword, item1.headword_pos), (item2.headword, item2.headword_pos))
        ...
        while not parser_state.done_parsing():
            ...
            if have_gold_actions:
                ...
                if action == 0:
                    ...
                    edge = parser_state.reduce_left()
                    actions_done.append(Actions.REDUCE_L)
                    dep_graph.add(edge)
                else:
                    edge = parser_state.reduce_right()
                ...
                if parser_state.stack_len() < 2:
                    ...
                    if index == 1:
                        ...
                        if parser_state.input_buffer_peek_n(1)[0].headword == END_OF_INPUT_TOK:
                            edge = parser_state.reduce_right()
                            actions_done.append(Actions.REDUCE_R)
                            dep_graph.add(edge)
                        else:
